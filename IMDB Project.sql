...
SELECT 
	  ...
	  COUNT(CASE 
			WHEN known_for_movies IS NULL THEN id END) AS known_for_movies_nulls
FROM
names;
...
FROM 
    ...
    genre as g
    ON g.movie_id = m.id
WHERE g.genre IN (SELECT DISTINCT genre FROM top_rated_genres WHERE genre_rank<=3) AND avg_rating	>8
GROUP BY name 
ORDER BY movie_count DESC 
LIMIT 3;
...
LIMIT 2;
	...
	*
	...
	SELECT 
		  ...
		  COUNT(m.id) as movie_count,
		  ROUND(
				SUM(r.avg_rating*r.total_votes) / SUM(r.total_votes),2) AS actor_avg_rating
	FROM 
		 names AS n
		 INNER JOIN
		 role_mapping AS a 
	...
	CASE 
		...
		THEN 'Hit movies' WHEN r.avg_rating BETWEEN 5 AND 7 
	...
	WHERE LOWER(g.genre) = 'thriller' AND r.total_votes > 25000;
	...
	top_grossing AS (
		SELECT
			...
			RANK() OVER (PARTITION BY g.genre, m.year ORDER BY CONVERT(REPLACE(TRIM(m.worlwide_gross_income), "$", ""), UNSIGNED INT) DESC) AS movie_rank
		FROM movie AS m
		INNER JOIN genre AS g ON g.movie_id = m.id
		WHERE g.genre IN (SELECT genre FROM top_genres WHERE genre_rank <= 3)
	)
	SELECT *
	FROM top_grossing
	WHERE movie_rank <= 5
	...
	FROM
		...
		ON m.id = r.movie_id
	WHERE 
		 median_rating>=8 AND m.production_company IS NOT NULL AND POSITION(',' IN languages)>0
	GROUP BY m.production_company 
	) 
	SELECT 
	*
...
WITH actress_ratings AS (
    SELECT 
        ...
        ROUND(SUM(r.avg_rating * r.total_votes) / SUM(r.total_votes), 2) AS actress_avg_rating
    FROM
        names AS n  -- Changed to 'names' table
    INNER JOIN
        role_mapping AS a ON n.id = a.name_id
    INNER JOIN
        movie AS m ON a.movie_id = m.id
...
WITH director_details AS (
    SELECT 
        ...
        AVG(DATEDIFF(m2.date_published, m.date_published)) AS avg_inter_movie_days,
    ...
    INNER JOIN movie m ON dm.movie_id = m.id  -- Linking movie to director
    INNER JOIN ratings r ON m.id = r.movie_id
    LEFT JOIN movie m2 ON m.id = m2.id AND m2.date_published > m.date_published  -- Calculate inter-movie days
    INNER JOIN names n ON dm.name_id = n.id  -- Joining with the 'names' table to get the director's name
    GROUP BY n.name  -- Group by director's name
),
ranked_directors AS (
    SELECT 
        director_name,
